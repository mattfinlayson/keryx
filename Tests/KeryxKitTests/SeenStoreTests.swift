...
struct InboxControllerPersistenceTests {
    ...
    func newFilesUnseen() throws {
        ...
        #expect(relaunched.state.unseenPaths.contains(dir.appendingPathComponent("job-1.md").path))
    }
    ...
    func openedPersists() throws {
        ...
        #expect(store.seenPaths.contains(dir.appendingPathComponent("job-1.md").path))

        let relaunched = makeController()
        try relaunched.refresh()

        #expect(relaunched.state.badgeCount == 1)
        #expect(relaunched.state.unseenPaths.contains(dir.appendingPathComponent("job-2.md").path))
        #expect(!relaunched.state.unseenPaths.contains(dir.appendingPathComponent("job-1.md").path))
    }
    ...
    func seededSeenStaysRead() throws {
        defaults.set([dir.appendingPathComponent("ghost.md").path], forKey: "seenPaths")
